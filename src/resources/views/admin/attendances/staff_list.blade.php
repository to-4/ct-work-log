...
<main class="attendance-list">
    <h1 class="attendance-list__title">{{ str_replace([' ', '　'], '', $user->name) }}さんの勤怠</h1>

    {{-- 月切り替え --}}
    <div class="month-switch-wrapper">
        <div class="month-switch">
            <a href="{{ route('admin.attendance.staff_list', ['id'=>$user->id, 'month' => $prevMonth->format('Y-m')]) }}" class="month-switch__btn month-switch__btn--left">
            ...
            <a href="{{ route('admin.attendance.staff_list', ['id'=>$user->id, 'month' => $nextMonth->format('Y-m')]) }}" class="month-switch__btn month-switch__btn--right">
